...
test("admin CRUD flow and RTL language switch", async ({ page }, testInfo) => {
  ...
  try {
    ...
    createdProjectId = createProjectPayload.data?.id ?? null;
    ...
    try {
      if (!createdProjectId) {
        const searchResponse = await page.request.get(
          `/api/admin/projects?search=${encodeURIComponent(projectSlug)}`,
        );
        ...
        if (searchResponse.ok()) {
          ...
          const matchedProject = searchPayload.data?.items?.find(
            (item) => item.slug === projectSlug,
          );
          createdProjectId = matchedProject?.id ?? null;
        }
      }

      if (createdProjectId) {
        await page.request.delete(`/api/admin/projects/${createdProjectId}`);
      }
    } catch {
      // Cleanup should never mask the main test result.
    }
  }
});
